 ...
 * @source $URL: http://svn.geotools.org/tags/2.4.4/modules/plugin/shapefile/src/main/java/org/geotools/index/quadtree/fs/FileSystemNode.java $
 */
public class FileSystemNodeQuad extends NodeQuad {
     ...
     */
    FileSystemNodeQuad(Envelope bounds, int id, NodeQuad parent, ScrollingBuffer buffer, int startByte, int subNodesLength) {
    ...
    public NodeQuad copy() throws IOException {
    	FileSystemNodeQuad copy = new FileSystemNodeQuad(getBounds(), id, getParent(), buffer, subNodeStartByte, subNodesLength );
    ...
    /**
     * @see org.geotools.INode.quadtree.Node#getSubNode(int)
     */
    public NodeQuad getSubNode(int pos) throws StoreException {
        ...
        try {
            FileSystemNodeQuad subNode = null;

            // Getting prec subNode...
            int offset = this.subNodeStartByte;

            if (pos > 0) {
                subNode = (FileSystemNodeQuad) getSubNode(pos - 1);
     ...
     * @throws IOException
     */
    public static FileSystemNodeQuad readNode(int id, NodeQuad parent, FileChannel channel, ByteOrder order)
        throws IOException {
        ScrollingBuffer buffer = new ScrollingBuffer(channel, order);
        return readNode(id, parent, buffer);
    }

    static FileSystemNodeQuad readNode(int id, NodeQuad parent, ScrollingBuffer buf)
        throws IOException {
        // offset 
        ...
        FileSystemNodeQuad node = new FileSystemNodeQuad(env, id, parent, buf, (int) buf.getPosition(), offset);
    ...
    /**
     * A utility class to access file contents by using a single scrolling buffer reading
     * file contents with a minimum of 8kb per access
     */
    private static class ScrollingBuffer {
        ...
        public void getIntArray(int[] array)  throws IOException {
            ...
            intView.get(array);
            // don't forget to update the original buffer position, since the view is independent
        ...
        public void goTo(long newPosition) throws IOException {
            // if the new position is already in the buffer, just move the buffer position
