 ...
 * 
 * <i>Required parameters during a read:</i>
 * <ul>
 * <li>An open and connected {@link SeConnection} used to suck out data from
 * ArcSDE for this image read operation.(setConnection())</li>
 * <li>A {@link HashMap} with {@link Integer} keys equal to the
 * SeObjectId.longValue() of each {@link SeRasterBand} which is to be read.
 * Values for each key should be the integer value of the band in the output
 * image to which the given SeRasterBand should be copied.<br/><br/>For
 ...
 * map as follows:<br/> Map m = new HashMap();<br/> m.put(new Integer(234),
 * new Integer(1));<br/> m.put(new Integer(234), new Integer(2));<br/>
 * m.put(new Integer(234), new Integer(3));<br/> m.put(new Integer(234), new
 * Integer(0));<br/> <br/> Note that the band indexes you choose will depend on
 ...
 * has its bands in the order "Blue (0), Green (1), Red (2)".<br/> </li>
 * <li>A (possibly null) {@link BufferedImage}, passed in via the
 * 'setDestination()' method. Make sure your bandmapper and this image contain
 * the same number of bands, and that you've mapped your output appropriately.<br/>
 * <i><b>If you leave this parameter null, a BufferedImage.TYPE_INT_ARGB will
 * be created, of exactly the size to cover your requested source region</b></i></li>
 * <li>An int[] of requested source bands from ArcSDE, set via
 ...
 * raster, use a setSourceBands() call like the following:<br/>
 * param.setSourceBands(new int[] { 2, 3, 4 });<br/> <b>A source band with the
 * value of zero won't work! start with 1!</b> </li>
 * <li>A {@link Rectangle} defining the source area of the raster level you
 ...
 * accordingly and only the proper bit of the raster will be loaded.</li>
 * </ul>
 * 
 * @author sfarber
 * 
...
public class ArcSDERasterImageReadParam extends ImageReadParam {
    ...
    protected HashMap bandMapper;

    public HashMap getBandMapper() {
        return bandMapper;
    }

    public void setBandMapper(HashMap bandMapper) {
