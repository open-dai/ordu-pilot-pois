...
public class GeometryEncoderSDE implements FilterVisitor {
    ...
    private void addSpatialFilter(final BinarySpatialOperator filter, final int sdeMethod,
        ...
        if (log.isLoggable(Level.FINER)) {
            log.finer("adding spatial filter " + filter + " method: " + sdeMethod + ", truth: "
                    + appliedTruth);
        }
        ...
        if(log.isLoggable(Level.FINEST)){
            log.fine("geometry literal: " + geom);
        }
        ...
        if(log.isLoggable(Level.FINEST)){
            log.finest("layer extent: " + seExtent);
        }
        ...
        try {
            ...
            log.finest("clipping geometry to layer extent...");
            geom = geom.intersection(layerEnv);
            if(log.isLoggable(Level.FINEST)){
                log.info("clipped geometry: " + geom);
            }
        } catch (SeException se) {
            throw new RuntimeException(new ArcSdeException(se).getMessage(), se);
        } catch (IOException e) {
...
//            }
            ...
            ArcSDEGeometryBuilder gb = ArcSDEGeometryBuilder.builderFor(geom.getClass());
            if(log.isLoggable(Level.FINEST)){
                log.finest("building SeShape for " + geom);
            }
            filterShape = gb.constructShape(geom, this.sdeLayer.getCoordRef());

            if(log.isLoggable(Level.FINEST)){
                log.finest("SeShape: " + filterShape);
            }
            
            // Add the filter to our list
            log.finest("creating SeShapeFilter");
            ...
            log.finest("SeShapeFilter created");
