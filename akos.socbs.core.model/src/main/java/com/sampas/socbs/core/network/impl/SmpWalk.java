...
import com.sampas.socbs.core.network.INodeSequence;
 ...
 *
 * @source $URL: http://svn.geotools.org/tags/2.4.4/modules/extension/graph/src/main/java/org/geotools/graph/path/Walk.java $
 */
public class SmpWalk extends ArrayList implements INodeSequence {
  ...
  //TODO: DOCUMENT ME!
  public SmpWalk(Collection nodes) {
    super(nodes);  
  }
  ...
  /**
   * @see INodeSequence#getLast()
   */
  public INode getLast() {
    return((INode)get(size()-1));
  }
  ...
  protected List buildEdges() {
    ...
    for (int i = 1; i < size(); i++) {
      INode prev = (INode)get(i-1);
      INode curr = (INode)get(i);
      
      IEdge e = curr.getEdge(prev); 
  ...
  //TODO: DOCUMENT ME!!!
  public SmpPath duplicate() {
    return(new SmpPath(this));  
  }
   
  public boolean equals(Object other) {
	  if (other instanceof SmpWalk) return(equals((SmpWalk)other));
	  return(false);	
	}
	
	public boolean equals(SmpWalk other) {
	  if (other.size() == size()) {
	    ...
	    while(thisnodes.hasNext()) {
	      INode thisnode = (INode)thisnodes.next();
	      INode othernode = (INode)othernodes.next();
