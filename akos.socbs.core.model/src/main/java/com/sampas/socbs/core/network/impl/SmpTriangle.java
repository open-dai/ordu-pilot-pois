 ...
 * @author jfc173
 */
public class SmpTriangle {
    
    IEdge edge1, edge2, edge3;
    ...
    public SmpTriangle(IEdge e1, IEdge e2, IEdge e3) {
        INode n1a = e1.getNodeA();
        INode n1b = e1.getNodeB();
        INode n2a = e2.getNodeA();
        INode n2b = e2.getNodeB();
        INode n3a = e3.getNodeA();
        INode n3b = e3.getNodeB();
        
        INode temp1, temp2, temp3;
        if (n1a.equals(n2a)){
            temp1 = n1a; //==n2a
            temp2 = n1b;
            temp3 = n2b;
    ...
    private void assign(IEdge e1, IEdge e2, IEdge e3, INode n1, INode n2, INode n3){
        edge1 = e1;
        edge2 = e2;
        edge3 = e3;
        node1 = n1;
    ...
    private void whinge(IEdge e1, IEdge e2, IEdge e3){
        throw new RuntimeException("You didn't give me a proper triangle.  " + e1 + ", " + e2 + ", " + e3);
    }
    
    public int relate(IXYNode n){
        int ret;
        if ((!(node1 instanceof IXYNode)) ||
            (!(node2 instanceof IXYNode)) ||
            (!(node3 instanceof IXYNode))){
            throw new RuntimeException("I can't perform a relate function on a non-spatial triangle");
        }
        LinearRing lr = fact.createLinearRing(new Coordinate[]{((IXYNode)node1).getCoordinate(), ((IXYNode)node2).getCoordinate(), ((IXYNode)node3).getCoordinate(), ((IXYNode)node1).getCoordinate()});
        Polygon poly = fact.createPolygon(lr, null);
        Point nPoint = fact.createPoint(n.getCoordinate());
        
        Line2D.Double line12 = new Line2D.Double(((IXYNode)node1).getCoordinate().x, ((IXYNode)node1).getCoordinate().y, ((IXYNode)node2).getCoordinate().x, ((IXYNode)node2).getCoordinate().y);
        Line2D.Double line13 = new Line2D.Double(((IXYNode)node1).getCoordinate().x, ((IXYNode)node1).getCoordinate().y, ((IXYNode)node3).getCoordinate().x, ((IXYNode)node3).getCoordinate().y);
        Line2D.Double line23 = new Line2D.Double(((IXYNode)node2).getCoordinate().x, ((IXYNode)node2).getCoordinate().y, ((IXYNode)node3).getCoordinate().x, ((IXYNode)node3).getCoordinate().y);
    ...
    public IEdge getBoundaryEdge(IXYNode n){
        if (!(relate(n) == ON_EDGE)){
        ...
        Line2D.Double line1 = new Line2D.Double(((IXYNode) edge1.getNodeA()).getCoordinate().x, ((IXYNode) edge1.getNodeA()).getCoordinate().y,
                                                ((IXYNode) edge1.getNodeB()).getCoordinate().x, ((IXYNode) edge1.getNodeB()).getCoordinate().y);
        Line2D.Double line2 = new Line2D.Double(((IXYNode) edge2.getNodeA()).getCoordinate().x, ((IXYNode) edge2.getNodeA()).getCoordinate().y,
                                                ((IXYNode) edge2.getNodeB()).getCoordinate().x, ((IXYNode) edge2.getNodeB()).getCoordinate().y);
        Line2D.Double line3 = new Line2D.Double(((IXYNode) edge3.getNodeA()).getCoordinate().x, ((IXYNode) edge3.getNodeA()).getCoordinate().y,
                                                ((IXYNode) edge3.getNodeB()).getCoordinate().x, ((IXYNode) edge3.getNodeB()).getCoordinate().y);   
        IEdge ret = null;
    ...
    public INode getThirdNode(IEdge e){
    ...
    public IEdge getOppositeEdge(INode n){
    ...
    public IEdge getSharedEdge(SmpTriangle t){
        IEdge[] tEdges = t.getEdges();
        IEdge shared = null;
        for (int i = 0; i < 3; i++){
    ...
    public IEdge[] getEdges(){
        return new IEdge[]{edge1, edge2, edge3};
    }
    
    public INode[] getNodes(){
        return new INode[]{node1, node2, node3};
    }
    ...
    public double getArea(){
        if ((node1 instanceof IXYNode) ||
            (node2 instanceof IXYNode) ||
            (node3 instanceof IXYNode)){
            double x1 = ((IXYNode) node1).getCoordinate().x;
            double y1 = ((IXYNode) node1).getCoordinate().y;
            double x2 = ((IXYNode) node2).getCoordinate().x;
            double y2 = ((IXYNode) node2).getCoordinate().y;
            double x3 = ((IXYNode) node3).getCoordinate().x;
            double y3 = ((IXYNode) node3).getCoordinate().y;
    ...
    public boolean equals(Object o){
        boolean ret;
        if (o instanceof SmpTriangle){
            ret = ((this.containsEdge(((SmpTriangle) o).getEdges()[0])) &&
                   (this.containsEdge(((SmpTriangle) o).getEdges()[1])) &&
                   (this.containsEdge(((SmpTriangle) o).getEdges()[2])));
        } else {
            ret = false;
        }
        return ret;
